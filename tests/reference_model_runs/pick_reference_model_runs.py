...
from pick_model_runs_fun import (
    load_packages_verbose,
    run_and_compare_h_cbc,
    test_diff_list,
)
...
for b in basenamepaths:
    ...
    report_direct[modelname] = test_diff_list(
        modelname,
        test_model_outputref_dir,
        test_model_inputdirect_dir,
        test_model_outputdirect_dir,
...
with open(
    os.path.join(test_example_dir, "../data", "MF2005: failed tests step 2.txt"),
    mode="w",
) as fh:
    ...
    for _k, v in report_direct.items():
...
with open(
    os.path.join(test_example_dir, "../data", "MF2005: succesful tests step 2.txt"),
    mode="w",
) as fh:
...
with open(
    os.path.join(test_example_dir, "../data", "MF2005: failed tests step 3.txt"),
    mode="w",
) as fh:
    fh.write("Failed models: " + ", ".join([k for k, v in report_direct.items() if not v["suc"]]) + "\n")
...
with open(
    os.path.join(test_example_dir, "../data", "MF2005: succesful tests step 3.txt"),
    mode="w",
) as fh:
    fh.write("Succesful models: " + ", ".join(suc_model_runs) + "\n")
